...
server/          Express + WebSocket host
  ...
  hostPage.ts    Desktop QR page rendered at /host
  input/         Translates input packets into bridge commands
  native/        InputBridge.cs — Win32 SendInput bridge, compiled to server/bin
...
The phone never talks to the internet. It loads the PWA from your PC over the local network and opens a WebSocket back to it; every tap and swipe becomes a Win32 `SendInput` call on the desktop.

Because the server is plain HTTP on your LAN, **anyone on the same network who opens the URL can control your mouse and keyboard.** Use it on your home or personal hotspot network, not on public or office Wi-Fi, and press **Stop mPad server** on the host page when you are done.
